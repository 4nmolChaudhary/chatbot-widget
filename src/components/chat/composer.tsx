...
const Composer = () => {
  ...
  const showAudioRecorder = config?.composer?.isAudioEnabled?.visible ?? true
  const placeholder = config?.composer?.placeholder?.value ?? 'Type your message...'
    ...
    <div className='w-full p-4 pt-0'>
      <div className='w-full inline-flex items-center justify-center whitespace-nowrap font-medium transition-all disabled:pointer-events-none disabled:opacity-50 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive border bg-card shadow-sm rounded-md gap-1.5 p-1'>
        <form onSubmit={onSubmit} className='relative w-full'>
          <textarea id='input' ref={textareaRef} value={messageInput} onChange={e => setMessageInput(e.target.value)} onKeyDown={handleKeyDown} placeholder={placeholder} className='w-full h-full font-normal px-3 py-2 text-sm outline-none disabled:cursor-not-allowed pr-12 resize-none overflow-hidden border-none' rows={2} />
          {!isRecording && (
